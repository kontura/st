...
thistlinelen(int y)
{
	...
	if (term.hist[y][i - 1].mode & ATTR_WRAP)
		return i;

	while (i > 0 && term.hist[y][i - 1].u == ' ')
		--i;
...
externalpipe(const Arg *arg)
{
	...
	newline = 0;
    // Add history
	for (n = 0; n < term.histi; n++) {
		bp = term.hist[n];
		lastpos = MIN(thistlinelen(n) + 1, term.col) - 1;
		if (lastpos < 0)
			break;
		end = &bp[lastpos + 1];
		for (; bp < end; ++bp)
			if (xwrite(to[1], buf, utf8encode(bp->u, buf)) < 0)
				break;
		if ((newline = term.hist[n][lastpos].mode & ATTR_WRAP))
			continue;
		if (xwrite(to[1], "\n", 1) < 0)
			break;
		newline = 0;
